...
const navLinks = [
  { label: "Home", path: "/" },
  {
    ...
    children: [
      ...
      { label: "Generative Search Optimization", path: "/services/generative-search-optimization" },
      { label: "Solar Panel Installation", path: "/services/solar-panel-installation" },
      { label: "Motion Graphics, 2D & 3D", path: "/services/motion-graphics" },
    ],
  },
        ...
        {mobileOpen && (
          ...
          >
            <div className="container mx-auto px-6 py-6 space-y-2">
                ...
                link.children ? (
                  <div key={link.label}>
                    <button
                      ...
                      className="w-full text-left px-4 py-3 text-muted-foreground font-medium flex items-center justify-between"
                    >
                      {link.label}
                    ...
                    <AnimatePresence>
                      {servicesOpen && (
                        <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden pl-4">
                          {link.children.map((child) => (
                            <Link key={child.path} to={child.path} className="block px-4 py-2.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
                              {child.label}
                            </Link>
                          ))}
                        </motion.div>
              ...
              <Link to="/contact" className="block mx-4 mt-4 text-center px-5 py-3 rounded-lg bg-primary text-primary-foreground font-semibold">
                Get Started
              </Link>
            </div>
          </motion.div>
